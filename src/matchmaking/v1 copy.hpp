...
namespace v1 {
    ...
    private:
        ...
        int evaluate(Checkers& board) {
            ...
            // if (board.isDarkTurn()) {
            //     features = NNUE::encodeBoard(board.getDarkPieces(), board.getLightPieces(), board.getKingPieces());
            // }
            // else {
            //     features = NNUE::encodeBoard(flipBoard(board.getLightPieces()), flipBoard(board.getDarkPieces()), flipBoard(board.getKingPieces()));
            // }
            // float output{ m_nnue.forward(Matrix{ features })[0, 0] };
    ...
    public:
        // AIPlayer(Checkers& board, EGTB& egtb, NNUE& nnue) : m_board(board), m_egtb(egtb), m_nnue(nnue), m_nodesHit(0), tt(ttSize, { 0, -1, 0, -1, 0 }) {}
